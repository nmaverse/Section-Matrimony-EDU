...
export default function FaqSection({ faqItems }: FaqSectionProps) {
  ...
  return (
    <section id="faq-section" className="relative py-24 bg-charcoal-dark border-t border-rose-gold/10 overflow-hidden">
              ...
              <div
                ...
                className={`rounded-2xl border transition-all duration-300 overflow-hidden ${
                  isOpen
                    ...
                    : 'border-rose-gold/10 bg-charcoal-mid/40 hover:border-rose-gold/20'
                }`}
              >
                ...
                >
                  <div className="flex items-center gap-4 pr-4">
                    <HelpCircle className={`w-5 h-5 shrink-0 transition-colors ${isOpen ? 'text-rose-gold' : 'text-neutral-500 group-hover:text-rose-gold'}`} />
                    <span className="font-display text-base md:text-lg font-semibold text-neutral-900 dark:text-white group-hover:text-rose-gold/90 transition-colors">
                      {item.question}
                    </span>
                  </div>

                  <div className={`p-1.5 rounded-full border shrink-0 transition-all ${isOpen ? 'border-rose-gold text-rose-gold bg-rose-gold/5' : 'border-neutral-200 dark:border-neutral-800 text-neutral-500'}`}>
                ...
                >
                  <div className="p-6 text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed font-light bg-charcoal-dark/40">
                    <p>{item.answer}</p>
                  </div>
                </div>
