            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl border border-rose-gold/10 bg-rose-gold/5 backdrop-blur-xl">
              ...
              <div className="flex flex-wrap gap-2.5">
                <button
                  ...
                  disabled={isLoading}
                  className="inline-flex h-9 items-center justify-center gap-1.5 px-4 rounded-xl border border-rose-gold/25 text-rose-gold text-xs font-semibold hover:bg-rose-gold/10 transition-all cursor-pointer disabled:opacity-40 disabled:pointer-events-none"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin' : ''}`} />
                  {isLoading ? 'Processing...' : 'Restore Student Defaults'}
                </button>
                ...
                {confirmClear ? (
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      disabled={isLoading}
                      className="inline-flex h-9 items-center justify-center gap-1.5 px-3.5 rounded-xl bg-rose-gold hover:bg-dusty-pink text-white text-xs font-semibold transition-all cursor-pointer shrink-0 disabled:opacity-40 disabled:pointer-events-none"
                    >
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className="inline-flex h-9 items-center justify-center px-3 rounded-xl border border-neutral-800 text-neutral-400 text-xs font-semibold hover:bg-[#141418] transition-all cursor-pointer disabled:opacity-40 disabled:pointer-events-none"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="inline-flex h-9 items-center justify-center gap-1.5 px-4 rounded-xl bg-rose-gold/10 hover:bg-rose-gold/15 text-rose-gold border border-rose-gold/20 text-xs font-semibold transition-all cursor-pointer disabled:opacity-40 disabled:pointer-events-none"
                  >
            ...
            {requests.length === 0 ? (
              ...
              <div className="border border-neutral-900 rounded-2xl overflow-hidden bg-charcoal-mid/20 backdrop-blur-sm">
                    ...
                    <tbody className="divide-y divide-neutral-900 font-light">
                      {requests.map((req) => (
                        <tr key={req.id} className="hover:bg-rose-gold/2 transition-colors">
                          ...
                          </td>                           {/* Individual DELETE with clear visual indicator */}
                          <td className="px-6 py-4 text-right">
                            <button
                              type="button"
                              onClick={() => onDeleteRequest(req.id)}
                              disabled={isLoading}
                              className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-lg border border-rose-gold/20 hover:border-red-500/40 text-rose-gold hover:text-red-400 hover:bg-red-500/10 transition-all font-sans font-medium text-xs cursor-pointer group disabled:opacity-40 disabled:pointer-events-none"
                            ...
                            >
                              ...
                              <span>{isLoading ? 'Removing...' : 'Delete Request'}</span>
                            </button>
                          </td>
                        </tr>
                      ))}
                ...
                <div className="px-6 py-4 bg-[#0a0a0c] border-t border-neutral-900 flex justify-between items-center text-[10px] text-neutral-500 font-mono">
                  ...
                  <span>Database Key: Cloud Firestore Realtime Sync</span>
                </div>
              </div>
            )}
